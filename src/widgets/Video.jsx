    ...
    return (
        <>
            <div className='list_container'>
                <div className='title_bar'><span className='tag_title'>{title}<span onClick={() => {
                ...
                }} className='playall'>全部播放</span></span>{back && <span onClick={() => { navigate(-1) }} className='float_right'>back</span>}</div>
                {
                    ...
                    <ul style={{ overflow: 'auto', height: '100%' }} id='haha' >
                        {
                            ...
                            songlist.map(item => <li
                                key={item.id}
                                onClick={() => { navigate(`/video/${item.vid}`) }}>
                        ...
                        <li ref={lastref} id='test' style={{ border: '1px solid red', height: '30px', textAlign: 'center' }} onClick={(evt) => { }}>{isloading ? <div>加载中</div> : hasmore.current ? '加载更多' : '没有更多了'}</li>
                    </ul>
                }
...
}), { saveSongInfo, toggle, getsongurl, savePlaylist, setIndex, toggleManualyFlag })(Video)
