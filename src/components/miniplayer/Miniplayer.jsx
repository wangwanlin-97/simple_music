...
import { toggle, changebig, savePlaylist, setIndex, toggleIsShow, saveSongInfo, toggleManualyFlag } from '../../redux/store';
...
function Miniplayer(props) {
    ...
    const { currentIndex, isplaying, toggle, changebig, songInfo, playInfo, playlist, toggleIsShow, isShow, setIndex, toggleManualyFlag, manualyFlag } = props
    ...
    useEffect(() => {
        if (canvasref.current) {
            ...
            ctx.lineWidth = '1'
            ctx.beginPath()
            ctx.arc(15, 15, 12, -1 * Math.PI / 2, 2 * Math.PI * progress - Math.PI / 2, false)
    ...
    return (
        <>
            <div className='miniplayer'>
                <i onClick={() => { changebig(true) }} className={isplaying ? 'rotate mini_img' : 'paused mini_img'} style={{ background: `url("${songInfo.picUrl}")center / 50px 50px `, overflow: 'hidden' }}
                // src={`${songInfo.picUrl}`}
                />
                <div className='minibar'>
                    ...
                    <div className='minicontroll'>
                        ...
                        <div onClick={() => {
                            ...
                            <canvas className='canvas' height={'30px'} width={'30px'} ref={canvasref} >

                            </canvas>
                        </div>
                        <div onClick={() => { toggleIsShow(!isShow) }} className='icon-currentlist'></div>
            ...
            </div>
            {
                isShow && <ul className='list'>

                    {
                        // console.log(list)
                        playlist.map(item => {
                            return <li className={item.id == songInfo.id ? 'active' : ''} onClick={() => { setIndex(playlist.indexOf(item)); handleGetSongUrl(item.id, { name: item.name, author: item.al.name, picUrl: item.al.picUrl, id: item.id }) }} key={item.id}>{item.name}</li>
                        })
                    }
                </ul>
            }
...
}, { toggle, changebig, savePlaylist, setIndex, toggleIsShow, toggleManualyFlag })(Miniplayer)
