...
function Addsong(props) {
    ...
    useEffect(() => {
        axios.get(`/record/recent/song?id=${userinfo.account.id}`).then(res => {
            ...
            // toggleSearchlist(res.data.data.list)
        })
    }, [])
    ...
    const getinfo = useRef(() => {
        ...
        const temp = []
        test.current = []
        axios.get(`/cloudsearch?keywords=${ipref.current.value}&offset=${offset.current}`).then(res => {
    ...
    return (
        <div>
            <div style={{ width: '100vw', display: 'flex', position: 'sticky', top: 0, backgroundColor: 'white', zIndex: '3333' }}>
                <input style={{ flex: '1' }} ref={ipref}></input>
                <button style={{ width: '50px' }} onClick={() => {
                    // axios.get(`/search?keywords=${ipref.current.value}`).then(res => {
                    //     console.log(res.data)
            ...
            <div style={{ overflow: 'auto', width: '100%' }}>
                {
                    searchlist.length > 0 && <List listid={id} operation={true} more={more} getmore={() => { offset.current += 30; getinfo.current() }} data={searchlist} ></List>
                }
            </div>
